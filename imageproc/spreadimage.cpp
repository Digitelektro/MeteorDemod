...
std::map<std::string, cv::MarkerTypes> SpreadImage::MarkerLookup{{"STAR", cv::MARKER_STAR},
...
cv::Mat SpreadImage::mercatorProjection(const cv::Mat& image, const PixelGeolocationCalculator& geolocationCalculator, float scale, ProgressCallback progressCallback) {
    ...
    if(settings.drawReceiver()) {
        ...
        cv::drawMarker(newImage,
                       cv::Point2d(coordinate.x, coordinate.y),
...
/*cv::Mat SpreadImage::equidistantProjection(const cv::Mat &image, const PixelGeolocationCalculator &geolocationCalculator, float scale, ProgressCallback progressCallback)
{
    ...
    PixelGeolocationCalculator::CartesianCoordinateF topLeft = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateTopLeft(), center, geolocationCalculator.getSatelliteHeight(),
scale); PixelGeolocationCalculator::CartesianCoordinateF topRight = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateTopRight(), center,
geolocationCalculator.getSatelliteHeight(), scale); PixelGeolocationCalculator::CartesianCoordinateF bottomLeft = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateBottomLeft(),
center, geolocationCalculator.getSatelliteHeight(), scale); PixelGeolocationCalculator::CartesianCoordinateF bottomRight =
PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateBottomRight(), center, geolocationCalculator.getSatelliteHeight(), scale);
    ...
    for (int y = 0; y < imageWithGeorefHeight; y += 50) {
        for (int x = 0; x < image.size().width; x += 50) {
            const PixelGeolocationCalculator::CartesianCoordinateF p1 = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateAt(x, y), center,
geolocationCalculator.getSatelliteHeight(), scale);
    ...
    GIS::ShapeRenderer countryBorders(settings.getResourcesPath() + settings.getShapeBoundaryLinesFile(), cv::Scalar(settings.getShapeBoundaryLinesColor().B, settings.getShapeBoundaryLinesColor().G,
settings.getShapeBoundaryLinesColor().R)); countryBorders.setThickness(settings.getShapeBoundaryLinesThickness()); countryBorders.drawShapeEquidistant(newImage, xStart, yStart, centerLatitude, centerLongitude, scale);

    GIS::ShapeRenderer cities(settings.getResourcesPath() + settings.getShapePopulatedPlacesFile(), cv::Scalar(settings.getShapePopulatedPlacesColor().B, settings.getShapePopulatedPlacesColor().G,
settings.getShapePopulatedPlacesColor().R)); cities.setFontScale(settings.getShapePopulatedPlacesFontScale()); cities.setThickness(settings.getShapePopulatedPlacesThickness());
    ...
    if(settings.drawReceiver()) {
        PixelGeolocationCalculator::CartesianCoordinateF coordinate = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>({settings.getReceiverLatitude(), settings.getReceiverLongitude(), 0}, {centerLatitude,
centerLongitude, 0}, mEarthRadius + mAltitude, scale); coordinate.x -= xStart; coordinate.y -= yStart; cv::drawMarker(newImage, cv::Point2d(coordinate.x, coordinate.y), cv::Scalar(0, 0, 0),
stringToMarkerType(settings.getReceiverMarkType()), settings.getReceiverSize(), settings.getReceiverThickness() + 1); cv::drawMarker(newImage, cv::Point2d(coordinate.x, coordinate.y), cv::Scalar(settings.getReceiverColor().B,
settings.getReceiverColor().G, settings.getReceiverColor().R), stringToMarkerType(settings.getReceiverMarkType()), settings.getReceiverSize(), settings.getReceiverThickness());
    }
...
cv::Mat SpreadImage::mercatorProjection(const std::list<cv::Mat>& images, const std::list<PixelGeolocationCalculator>& geolocationCalculators, float scale, SpreadImage::ProgressCallback progressCallback) {
    ...
    for(int i = 0; image != images.end(); ++image, ++geolocationCalculator, i++) {
        ...
        for(int y = 0; y < imageWithGeorefHeight - 10; y += 10) {
            ...
            for(int x = 0; x < image->size().width - 10; x += 10) {
                const PixelGeolocationCalculator::CartesianCoordinateF p1 = PixelGeolocationCalculator::coordinateToMercatorProjection<float>(geolocationCalculator->getCoordinateAt(x, y), geolocationCalculator->getSatelliteHeight(), scale);
                const PixelGeolocationCalculator::CartesianCoordinateF p2
                    = PixelGeolocationCalculator::coordinateToMercatorProjection<float>(geolocationCalculator->getCoordinateAt(x + 10, y), geolocationCalculator->getSatelliteHeight(), scale);
                const PixelGeolocationCalculator::CartesianCoordinateF p3
                    = PixelGeolocationCalculator::coordinateToMercatorProjection<float>(geolocationCalculator->getCoordinateAt(x, y + 10), geolocationCalculator->getSatelliteHeight(), scale);
...
cv::Mat SpreadImage::equidistantProjection(const cv::Mat& image, const PixelGeolocationCalculator& geolocationCalculator, float scale, ProgressCallback progressCallback) {
    ...
    for(int y = 0; y < imageWithGeorefHeight - 10; y += 10) {
        ...
        for(int x = 0; x < image.size().width - 10; x += 10) {
            const PixelGeolocationCalculator::CartesianCoordinateF p1
                = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateAt(x, y), center, geolocationCalculator.getSatelliteHeight(), scale);
            const PixelGeolocationCalculator::CartesianCoordinateF p2
                = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateAt(x + 10, y), center, geolocationCalculator.getSatelliteHeight(), scale);
            const PixelGeolocationCalculator::CartesianCoordinateF p3
                = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>(geolocationCalculator.getCoordinateAt(x, y + 10), center, geolocationCalculator.getSatelliteHeight(), scale);
    ...
    if(settings.drawReceiver()) {
        PixelGeolocationCalculator::CartesianCoordinateF coordinate
            = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>({settings.getReceiverLatitude(), settings.getReceiverLongitude(), 0}, {centerLatitude, centerLongitude, 0}, mEarthRadius + mAltitude, scale);
        coordinate.x -= xStart;
        coordinate.y -= yStart;
        cv::drawMarker(newImage, cv::Point2d(coordinate.x, coordinate.y), cv::Scalar(0, 0, 0), stringToMarkerType(settings.getReceiverMarkType()), settings.getReceiverSize(), settings.getReceiverThickness() + 1);
        cv::drawMarker(newImage,
...
cv::Mat SpreadImage::equidistantProjection(const std::list<cv::Mat>& images, const std::list<PixelGeolocationCalculator>& geolocationCalculators, float scale, SpreadImage::ProgressCallback progressCallback) {
    ...
    if(settings.drawReceiver()) {
        PixelGeolocationCalculator::CartesianCoordinateF coordinate
            = PixelGeolocationCalculator::coordinateToAzimuthalEquidistantProjection<float>({settings.getReceiverLatitude(), settings.getReceiverLongitude(), 0}, center, mEarthRadius + mAltitude, scale);
        coordinate.x -= xStart;
        coordinate.y -= yStart;
        cv::drawMarker(composite, cv::Point2d(coordinate.x, coordinate.y), cv::Scalar(0, 0, 0), stringToMarkerType(settings.getReceiverMarkType()), settings.getReceiverSize(), settings.getReceiverThickness() + 1);
        cv::drawMarker(composite,
...
void SpreadImage::affineTransform(const cv::Mat& src, cv::Mat& dst, const cv::Point2f source[], const cv::Point2f destination[], int originX, int originY) {
    ...
    for(int y = minY; y < maxY; y++) {
        for(int x = minX; x < maxX; x++) {
            ...
            cv::Vec3b interpolated = (src.at<cv::Vec3b>(y1, x2)) * ((srcX - x1) * (y2 - srcY)) / ((y2 - y1) * (x2 - x1)) + (src.at<cv::Vec3b>(y2, x1)) * (((srcY - y1) * (x2 - srcX)) / ((y2 - y1) * (x2 - x1)))
                                     + (src.at<cv::Vec3b>(y1, x1)) * (((x2 - srcX) * (y2 - srcY)) / ((y2 - y1) * (x2 - x1))) + (src.at<cv::Vec3b>(y2, x2)) * ((srcY - y1) * (srcX - x1)) / ((y2 - y1) * (x2 - x1));
